...
<header class="flex justify-between items-center p-6 text-white fixed top-0 left-0 w-full z-30">
  <div @click="goToMainDashboard" class="flex items-center gap-3 bg-white/20 px-4 py-2 rounded-xl cursor-pointer hover:bg-white/30 transition">
...
const { user, profile, fetchUserAndProfile, logout } = userStore;

const goToMainDashboard = () => router.push('/dashboard');
const handleLogout = () => logout(router);

onMounted(fetchUserAndProfile);
</script>